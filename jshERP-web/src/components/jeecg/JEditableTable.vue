      ...
      <div class="thead" ref="thead">
        <div class="tr" :style="{ width: this.realTrWidth }">
          ...
          <template v-for="col in columns" :key="col.key">
            <div v-show="col.type !== formTypes.hidden" class="td" :style="buildTdStyle(col)">
      ...
      <div class="scroll-view" ref="scrollView" :style="{ 'max-height': maxHeight + 'px' }">
        ...
        <div :id="`${caseId}tbody`" class="tbody" :style="tbodyStyle">
              ...
              >
                ...
                >
                  ...
                  <template v-for="(id, i) in [`${col.key}${row.id}`]">
                    ...
                    <div v-else-if="col.type === formTypes.upload" :key="`if-${i}`">
                      <template
                    ...
                    <div v-else-if="col.type === formTypes.file" :key="`else-if-${i}`">
                      <template
                        v-if="uploadValues[id] != null"
                        v-for="(file, fileKey) of [uploadValues[id] || {}]"
                        :key="fileKey"
                    ...
                    <div v-else-if="col.type === formTypes.image" :key="`else-if1-${i}`">
                      <template
                        v-if="uploadValues[id] != null"
                        v-for="(file, fileKey) of [uploadValues[id] || {}]"
                        :key="fileKey"
                    ...
                    <div v-else-if="col.type === formTypes.slot" :key="`else-if2-${i}`">
                      <a-tooltip
                        :key="i"
                        :id="id"
                        placement="top"
          ...
          >
            ...
            <div v-if="dragSortAndNumber" class="td td-ds" :style="style.tdLeftDs"></div>
            <div v-if="rowNumber" class="td td-num" :style="style.tdLeft">
              <span v-if="!rowSelection">统计</span>
            </div>
            <div v-if="rowSelection" class="td td-cb" :style="style.tdLeft"></div>

            <!-- 右侧动态生成td -->
            <template v-for="col in columns" :key="col.key">
              <div class="td" v-show="col.type !== formTypes.hidden" :style="buildTdStyle(col)">
                <span v-show="col.type === formTypes.inputNumber" style="padding: 0 2px">{{
...
export default {
  ...
  methods: {
    ...
    autoJumpNextInputBill() {
      ...
      inputDom
        .find('input:visible:not(:checkbox)')
        .off('keydown')
