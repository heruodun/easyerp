...
<script>
import { h } from 'vue' // 替代 createElement
...
export default {
  ...
  methods: {
    ...
    renderSaveTreeData(item) {
      ...
      item.icon = () => h('a-icon', { props: { type: 'file-text' } })
      item.originTitle = item['title']

      item.title = (_, vNode) => {
        return h('div', { class: 'j-history-tree-title' }, [
          h('span', vNode.dataRef.originTitle),
          h(
            'div',
            {
              class: 'j-history-tree-title-closer',
              on: {
                click: (e) => self.handleRemoveSaveTreeItem(e, vNode),
              },
            },
