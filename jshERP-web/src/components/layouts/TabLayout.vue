...
export default {
  ...
  isOpenIframePage() {
    const target = componentsArr.value.find((item) => item.path === route.path)
  ...
  onMounted() {
    const arr = getComponentsArr()
    arr.forEach((item) => {
      app?.component(item.name, item.component)
    })
    componentsArr.value = arr
    isOpenIframePage()

    if (route.path !== indexKey) {
      addIndexToFirst()
    }

    const storeKey = `route:title:${route.fullPath}`
    const routeTitle = ls.get(storeKey)
    if (routeTitle) {
      route.meta.title = routeTitle
    }

    pageList.value.push({ ...route })
    linkList.value.push(route.fullPath)
