  ...
  actions: {
        ...
        getAction('/cas/client/validateLogin', userInfo)
          .then((response) => {
            ...
            if (response.success) {
              ...
              ls.set(ACCESS_TOKEN, result.token, 7 * 24 * 60 * 60 * 1000)
              ls.set(USER_NAME, userInfo.username, 7 * 24 * 60 * 60 * 1000)
          ...
          .then((response) => {
            if (response.code === 200) {
              if (response.data.msgTip === 'user can login') {
                ...
                ls.set(USER_ID, result.user.id, 7 * 24 * 60 * 60 * 1000)
                ls.set(USER_LOGIN_NAME, result.user.loginName, 7 * 24 * 60 * 60 * 1000)
                //前端7天有效期，后端默认1天，只要用户在1天内有访问页面就可以一直续期直到7天结束
                ls.set(ACCESS_TOKEN, result.token, 7 * 24 * 60 * 60 * 1000)
                ls.set(USER_INFO, result.user, 7 * 24 * 60 * 60 * 1000)
                commit('SET_TOKEN', result.token)
              }
              commit('SET_INFO', userInfo)
              resolve(response)
    ...
    GetPermissionList({ commit }) {
      return new Promise((resolve, reject) => {
        ...
        let params = { pNumber: 0, userId: ls.get(USER_ID) }
