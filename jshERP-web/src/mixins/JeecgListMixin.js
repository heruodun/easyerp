...
export const JeecgListMixin = {
  ...
  methods: {
    ...
    //加载初始化列
    initColumnsSetting() {
      let columnsStr = ls.get(this.pageName)
    ...
    onColChange(checkedValues) {
      ...
      ls.set(this.pageName, columnsStr)
    },
    //恢复默认
    handleRestDefault() {
      ls.remove(this.pageName)
    ...
    /* 按钮权限 */
    initActiveBtnStr() {
      let btnStrList = ls.get('winBtnStrList') //按钮功能列表 JSON字符串
    ...
    handleChangeOtherField(showQuery) {
      let mpStr = getMpListShort(ls.get('materialPropertyList'))
      if (mpStr) {
