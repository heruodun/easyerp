      ...
      getAction(this.url.list, params).then((res) => {
        if (res.code === 200) {
          ...
          this.dataSource = this.dataSource.map((item) => {
            // 确保 receivableAmount 存在且初始化
            const receivableAmount = typeof item.receivableAmount === 'undefined' ? item.allNeed : item.receivableAmount

            return {
              ...item,
              receivableAmount: Math.min(receivableAmount, item.allNeed), // 确保不超过欠款金额
            }
          })
