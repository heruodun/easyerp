  ...
  methods: {
    ...
    waitNeedListOk(organType, organId, selectBillRows) {
      ...
      if (selectBillRows && selectBillRows.length > 0) {
        this.requestSubTableDataEx(selectBillRows, this.accountTable, false)
      } else {
    ...
    debtBillListOk(selectBillRows) {
      if (selectBillRows && selectBillRows.length > 0) {
        this.requestSubTableDataEx(selectBillRows, this.accountTable, true)
      }
    },
    /** 查询某个tab的数据,给明细里面的金额赋值 */
    requestSubTableDataEx(selectBillRows, tab, autoEachAmount, success) {
      tab.loading = true
      let listEx = []
      let changeAmount = 0
      ...
      for (let i = 0; i < selectBillRows.length; i++) {
        ...
        console.log('requestSubTableDataEx' + info.eachAmount)
        if (autoEachAmount) {
      ...
      if (btnStrList) {
        for (let i = 0; i < btnStrList.length; i++) {
          if (btnStrList[i].btnStr) {
            ...
            this.quickBtn.person =
              btnStrList[i].url === '/system/person' ? btnStrList[i].btnStr.indexOf(1) > -1 : this.quickBtn.person
            this.quickBtn.inOutItem =
