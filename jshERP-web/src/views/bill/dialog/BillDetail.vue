      ...
      <template v-else-if="billType === '销售出库'">
        <section ref="print" id="saleOutPrint">
          ...
          <a-row class="form-row" :gutter="24">
            ...
            <a-col :span="6">
              <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="此前欠款">
                {{ model.previousDebt }}
              </a-form-item>
            </a-col>

            <a-col :span="6">
              <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="累计欠款">
                {{ model.allNeed }}
              </a-form-item>
            </a-col>
  ...
  methods: {
    ...
    retailExportExcel() {
      ...
      let head =
        ...
        ',库存,单位,序列号,批号,有效期,多属性,数量,单价,金额,备注'
    ...
    purchaseSaleExportExcel() {
      ...
      let head =
        '仓库名称,条码,名称,规格,型号,颜色,' +
        this.otherFieldTitle +
        ',库存,单位,序列号,批号,有效期,多属性,数量,单价,金额,税率(%),税额,价税合计,重量,备注'
      for (let i = 0; i < this.dataSource.length; i++) {
        let item = []
        let ds = this.dataSource[i]
        item.push(
    ...
    allocationOutExportExcel() {
      let list = []
      let head =
        '仓库名称,条码,名称,规格,型号,颜色,' + this.otherFieldTitle + ',库存,调入仓库,单位,多属性,数量,单价,金额,备注'
      for (let i = 0; i < this.dataSource.length; i++) {
        let item = []
        let ds = this.dataSource[i]
        item.push(
    ...
    assembleExportExcel() {
      let list = []
      let head = [
        '商品类型,仓库名称,条码,名称,规格,型号,颜色,' + this.otherFieldTitle + ',库存,单位,多属性,数量,单价,金额,备注',
      ]
      for (let i = 0; i < this.dataSource.length; i++) {
        let item = []
        let ds = this.dataSource[i]
